  ...
  const bookingDetails = bookings.map((booking) => {
    ...
    if (booking.addons && booking.addons.length) {
      booking.addons.forEach((addonId) => {
        ...
        const addonStyleOption = allAddons.find((ser) =>
          ser.style_options.some((opt) => booking.addons?.includes(opt.id))
        );

        if (addonStyleOption) {
          addons.push(addonId);
          totalPrice += Number(addonStyleOption?.base_price) || 0;
          totalDuration += addonStyleOption?.duration || 0;
        }

        // Check if the addonId matches a variation.id
        const addonVariation = allAddons.find((ser) =>
          ser.variations.some((opt) => booking.addons?.includes(opt.id))
        );
        if (addonVariation) {
          addons.push(addonId);
          totalPrice += Number(addonVariation?.base_price) || 0;
